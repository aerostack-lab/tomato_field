...
tmux -2 new-session -d -s df_main_launcher2

# Create one window per roslaunch (-t -> target session , -n -> window name) 
tmux new-window -t df_main_launcher2:1 -n 'viewer + teleop'
...
tmux new-window -t df_main_launcher2:2 -n 'pixhawk interface'
...
tmux new-window -t df_main_launcher2:3 -n 'Basic Quadrotor Behaviors'
tmux send-keys "roslaunch basic_quadrotor_behaviors basic_quadrotor_behaviors.launch --wait \
  namespace:=drone$NUMID_DRONE" C-m

tmux new-window -t df_main_launcher2:4 -n 'Quadrotor Motion With DF Control'
...
tmux new-window -t df_main_launcher2:5 -n 'GroundTruth Gazebo'
...
tmux new-window -t df_main_launcher2:6 -n 'Throttle_Controller'
tmux send-keys "roslaunch thrust2throttle_controller thrust2throttle_controller.launch --wait \
  namespace:=drone$NUMID_DRONE \
  uav_mass:=$UAV_MASS" C-m

tmux new-window -t df_main_launcher2:7 -n 'Python interpreter'
...
tmux new-window -t df_main_launcher2:8 -n 'Belief Manager'
...
tmux new-window -t df_main_launcher2:11 -n 'Swarm behaviors'
tmux send-keys "roslaunch swarm_interaction swarm_interaction.launch --wait \
  namespace:=drone$NUMID_DRONE" C-m

tmux new-window -t df_main_launcher2:12 -n 'Belief memory Viewer'
tmux send-keys "roslaunch belief_memory_viewer belief_memory_viewer.launch  --wait \
  drone_id_namespace:=drone$NUMID_DRONE \
  drone_id:=$NUMID_DRONE" C-m
  
tmux new-window -t df_main_launcher2:13 -n 'Camera Raw'
tmux send-keys "rqt_image_view /iris_1/usb_cam/image_raw" C-m

tmux attach-session -t df_main_launcher2:1
